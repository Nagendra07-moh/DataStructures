...
void SUBSET_SUM(int ind,vector<int>vec,vector<int>&ans,vector<int> temp){
	if(ind == vec.size()){
		int sum =0;
		for(auto it:temp){
			sum +=it;
...
/*-------------------------------------------------------------------------------------------------------------*/
						/*ALL POSSIBLE PERMUTATIONS OF AN ARRAY(BRUTE_FORCE) (build a freq map with all 0's)*/

void ALL_PERMUTATIONS_BRUTE(vector<int>vec,vector<vector<int>>&ans,vector<int>temp,vector<bool>mp){
	...
	for(int i=0	;i<vec.size();i++){
		if(mp[i] == 0){
			temp.push_back(vec[i]);
			mp[i] = 1;
			ALL_PERMUTATIONS_BRUTE(vec,ans,temp,mp);
			mp[i] = 0;
			temp.pop_back();
		}
	}
...
/*-------------------------------------------------------------------------------------------------------------*/
			/*ALL POSSIBLE PERMUTATIONS OF AN ARRAY(OPTIMAL(without extra space)) (build a freq map with all 0's)*/

void ALL_PERMUTATIONS_OPTIMAL(int ind,vector<int>vec,vector<vector<int>>&ans,vector<int>temp){
	if(ind == vec.size()){
		ans.push_back(vec);
		return;
	}
	for(int i=ind;i<vec.size();i++){
		swap(vec[ind],vec[i]);
		ALL_PERMUTATIONS_OPTIMAL(ind+1,vec,ans,temp);
		swap(vec[ind],vec[i]);
	}
}
